...
contract MoonExtension is AdminControl, CreatorExtension, ICreatorExtensionTokenURI, IManifoldERC721Edition, AccessControl, ReentrancyGuard {
    ...
    function getFlipEngine() public view returns(address) {
        return address(flipEngine);
    }
    ...
    function formatTokenURI() public view returns (string memory) {
        ...
        string memory byteEncoded = Base64.encode(bytes(abi.encodePacked(
            ...
            '", "description": "', 
            _description, 
            '", "attributes": [{"trait_type":"ARTIST", "value":"omentejovem"},{"trait_type":"ENGINEER", "value":"andrew mitchell"}], "external_url": "', 
            _externalUrl, 
            '", "image": "',
             imageUri,
              '"}'
